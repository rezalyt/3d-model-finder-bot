...
async function searchSketchfab(query, format) {
  const params = new URLSearchParams({ q: query, type: 'models', limit: String(LIMIT), sort_by: '-relevance' });
  const headers = process.env.SKETCHFAB_API_TOKEN
...
async function searchPolyHaven(query, format) {
  ...
  return Object.entries(polyhavenCache)
    .filter(([id, item]) => [id, item.name, item.description, ...(item.tags || []), item.category]
      .filter(Boolean).join(' ').toLowerCase().split(/\s+/)
      .every(term => q.toLowerCase().split(/\s+/).every(t => term.includes(t)) || String(id).toLowerCase().includes(term)))
    .slice(0, LIMIT)
...
async function searchNasa(query, format) {
  ...
  return [...groups.entries()].slice(0, LIMIT).map(([name, formats]) => normalize({
    name,
...
async function searchSmithsonian(query, format) {
  ...
  return (data?.rows || data?.results || [])
    .map(item => normalize({
      ...
      formats: item.model_type ? [normFormat(item.model_type)] : (format ? [format] : []),
...
let fetch3d = null;
try {
  fetch3d = new Fetch3D({ timeout: TIMEOUT_MS });
} catch (error) {
...
async function searchFallback(query, format) {
  if (!fetch3d) return [];
  try {
    const opts = format ? { query, formats: [format], limit: LIMIT } : { query, limit: LIMIT };
    const result = await fetch3d.searchAll(opts);
    return (result?.models || []).map(item => normalize(item, item.source || '3dfetch-fallback'))
      .filter(item => !format || item.formats.includes(format));
  } catch (error) {
...
async function searchModels(query, format) {
  const tasks = [
    searchPrintables(query, format),
    searchSketchfab(query, format),
    searchPolyHaven(query, format),
    searchNasa(query, format),
    process.env.SMITHSONIAN_API_KEY ? searchSmithsonian(query, format) : Promise.resolve([]),
  ];
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    directProviders: ['printables','sketchfab','polyhaven','nasa','3ddd',...(process.env.SMITHSONIAN_API_KEY ? ['smithsonian'] : [])],
    fallback: '3dfetch',
