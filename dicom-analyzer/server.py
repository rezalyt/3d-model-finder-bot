...
def file_id(url):
    ...
    return m.group(1) if m else parse_qs(urlparse(url or "").query).get("id", [None])[0]
...
def to_hu(ds):
    a = ds.pixel_array.astype(np.float32); a = a * float(getattr(ds, "RescaleSlope", 1.0)) + float(getattr(ds, "RescaleIntercept", 0.0)); return np.nan_to_num(a[0] if a.ndim > 2 else a, nan=0.0, posinf=0.0, neginf=0.0)
def window(a, center, width):
    lo, hi = center - width / 2.0, center + width / 2.0; return Image.fromarray((np.clip((a - lo) / max(width, 1.0), 0, 1) * 255).astype(np.uint8), mode="L")
def panel(a):
    imgs = [ImageOps.fit(window(a, 40, 350), (THUMB_SIZE, THUMB_SIZE)), ImageOps.fit(window(a, -600, 1500), (THUMB_SIZE, THUMB_SIZE)), ImageOps.fit(window(a, 300, 2500), (THUMB_SIZE, THUMB_SIZE))]
    out = Image.new("RGB", (THUMB_SIZE * 3, THUMB_SIZE)); [out.paste(x.convert("RGB"), (i * THUMB_SIZE, 0)) for i, x in enumerate(imgs)]; return out

def read_series_inventory(zpath):
    series = {}
    with zipfile.ZipFile(zpath) as z:
        for info in z.infolist():
            if info.is_dir() or Path(info.filename).is_absolute() or ".." in Path(info.filename).parts: continue
            try:
                with z.open(info) as fh: ds = read_ds(fh.read(), False)
                if not (hasattr(ds, "SOPClassUID") and hasattr(ds, "Rows") and hasattr(ds, "Columns")): continue
                uid = str(getattr(ds, "SeriesInstanceUID", "unknown")); series.setdefault(uid, []).append({"name": info.filename, "instance": int(getattr(ds, "InstanceNumber", 0) or 0), "series_number": int(getattr(ds, "SeriesNumber", 0) or 0), "modality": str(getattr(ds, "Modality", "")), "body_part": str(getattr(ds, "BodyPartExamined", "")), "description": str(getattr(ds, "SeriesDescription", "")), "rows": int(getattr(ds, "Rows", 0) or 0), "cols": int(getattr(ds, "Columns", 0) or 0)})
...
def process_full(job_id, url):
    ...
    try:
        download(url, zpath); series = read_series_inventory(zpath)
            ...
            for idx, (uid, items) in enumerate(series.items(), 1):
                ...
                for n, item in enumerate(items, 1):
                    try:
                        ...
                        fname = f"slice_{n:05d}.jpg"; panel(a).save(sdir / fname, quality=72, optimize=True)
                ...
                if med:
                    for ratio, r in sorted([(s / med, r) for r, s in zip(slice_rows, scores)], reverse=True)[:20]: candidates.append({"index": r["index"], "instance": r["instance"], "outlier_ratio": round(float(ratio), 2)})
                base = items[0]; summaries.append({"series_index": idx, "series_uid": uid, "series_number": base["series_number"], "series_description": base["description"], "modality": base["modality"], "body_part": base["body_part"], "instances": len(items), "processed_slices": len(slice_rows), "rows": base["rows"], "cols": base["cols"], "viewer": f"/viewer/{job_id}/{idx}", "top_candidates": candidates, "slices": slice_rows})
...
def process_ai(job_id, url):
    ...
    try:
        ...
        series = read_series_inventory(zpath); eligible = []; skip_words = ("topogram", "scout", "localizer", "monitor", "premonitor")
        ...
        eligible.sort(reverse=True); _, uid, items = eligible[0]
        with zipfile.ZipFile(zpath) as z:
            for item in items:
        ...
        cmd = ["TotalSegmentator", "-i", str(input_nii), "-o", str(out), "--fast", "--device", "cpu", "--roi_subset", *AI_ROIS, "--force_split", "--nr_thr_resamp", "1", "--nr_thr_saving", "1", "--statistics", "--quiet"]
        with LOCK: JOBS[job_id] = {"job_id": job_id, "status": "ai_segmenting", "model": "TotalSegmentator 2.18.0", "device": "cpu", "memory_mode": "force_split_single_thread", "series_instances": len(items), "series_description": items[0]["description"], "rois": AI_ROIS}
        ...
        if proc.returncode != 0: raise RuntimeError(f"TotalSegmentator failed (returncode={proc.returncode}): " + (proc.stdout or "")[-6000:])
        stats_path = out / "statistics.json"; stats = json.loads(stats_path.read_text(encoding="utf-8")) if stats_path.exists() else {}
        result = {"job_id": job_id, "status": "completed", "model": "TotalSegmentator 2.18.0", "device": "cpu", "mode": "fast", "memory_mode": "force_split_single_thread", "series_description": items[0]["description"], "series_instances": len(items), "rois": AI_ROIS, "statistics": stats}
